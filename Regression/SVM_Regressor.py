...
class SVM_Regressor():
    """
An automated Support Vector Machine Regressor.
Parameters
----------
...
cols_list : list, optional (default="None")
    List of names of columns for which you want the Support Vector Machine
...
SVM_Train : Fits the Support Vector Machine Regressor model on the training data
...
save_files : Calls all other functions. It calculates the univariate distribution
    for each dataset and the correlation coefficient among all indepedent variables. It saves
    all of the above mentioned results in an excel file along with the order of variables which 
...
#            combine[combine.select_dtypes(include=np.number).columns] = combine[combine.select_dtypes(include=np.number).columns].fillna(value=99999999)
    ...
    def normalize(self):
        ...
        oos_copy = self.oos_data.copy()
        oot_copy = self.oot_data.copy()
        pdv_copy = self.pdv_data.copy()
        psi_copy = self.psi_data.copy()
        norm_cols = self.train_data.select_dtypes(include=np.number).columns
        minmax = MinMaxScaler()
        train_features = train_copy[norm_cols]
        oos_features=oos_copy[norm_cols]
        oot_features=oot_copy[norm_cols]
        pdv_features=pdv_copy[norm_cols]
        psi_features=psi_copy[norm_cols]
        scaler = minmax.fit(train_features.values)
        train_features = scaler.transform(train_features.values)
        if self.oos_data is not None:
            oos_features = scaler.transform(oos_features.values)
        oot_features = scaler.transform(oot_features.values)
        pdv_features = scaler.transform(pdv_features.values)
        psi_features = scaler.transform(psi_features.values)
        train_copy[norm_cols] = train_features
        oos_copy[norm_cols] = oos_features
        oot_copy[norm_cols] = oot_features
        pdv_copy[norm_cols] = pdv_features
        psi_copy[norm_cols] = psi_features
        self.train_data=train_copy
        self.oos_data=oos_copy
        self.oot_data=oot_copy
        self.pdv_data=pdv_copy
        self.psi_data=psi_copy
        del train_copy,oos_copy,oot_copy,pdv_copy,psi_copy
        
    def SVM_Train(self):
        self.clf=svm.SVR(C=1.0,gamma='scale', kernel='rbf')
...
project_name="SVM_Regressor"
location="D:\Project 8\SVM Regressor"

model1=SVM_Regressor(dev_data=dev_data,oos_data=oos_data,oot_data=oot_data,pdv_data=pdv_data,psi_data=psi_data,dependent=' duration',split_flag=False,hyperopt_flag=False,location=location,project_name=project_name,cols_list=cols_list)
